...
(function (global) {
  ...
  var Iso = global.Iso, World = global.World, Sim = global.Sim, Hedge = global.Hedge;
  var P = Iso.project;
  ...
  function drawVault(b) {
    ...
    var spin = Sim.state.station === 'clearing' ? t * 1.1 : 0;
    ctx.save();
  ...
  function drawStack(b) {
    ...
    var busy = Sim.state.station === 'exchange';
    for (var i = 0; i < 4; i++) {
  ...
  function drawDrums(b) {
    ...
    /* a settlement indicator: one lamp per assumed daily settlement */
    var s = Sim.state;
    if (s.station === 'settlement' || s.station === 'exposure') {
      var n = s.station === 'settlement' && s.result ? s.result.settlements.length : 5;
      for (var q = 0; q < n; q++) {
  ...
  /* -------------------------------------------------------------- the cart
     The side gauge is journey progress. Its cargo is one visible crate per
     modeled contract, capped at ten for legibility; the panel shows the exact
     count. Blue means a long hedge and brick means a short hedge. */
  ...
  function drawVan(v) {
    ...
    /* cab */
    Iso.orientedBox(ctx, { x: v.x + hx * 0.85, y: v.y + hy * 0.85, z: z + 0.5, hx: hx, hy: hy, len: 0.85, wid: 1.1, h: 0.76, color: s.scenario === 'producer' ? '#a85a44' : '#537f9e' });
    ...
    /* the cargo: one crate per contract, visually capped */
    if (s.cargoContracts > 0) {
      var crates = Math.max(1, Math.min(10, s.cargoContracts));
  ...
  function draw(canvas, camera, time, activeDistrict, hoverDistrict) {
    ...
    if (showLabels) {
      ...
      for (i = 0; i < World.districts.length; i++) {
        ...
        /* Once visited, each plate carries the live value from model.js. */
        var sub = isActive ? d.tag : null;
        if (s.visited && s.visited[d.id] && s.result) {
          var stage = Hedge.stageOf(s.result, d.id);
          if (stage) sub = stage.value;
        }
        labels.push({
    ...
    /* The live hedge state rides above the cart. */
    if (s.running) {
      labels.push({
        x: v.x, y: v.y, z: (v.z || 0) + 2.4, lift: 8,
        text: s.result ? s.result.sideLabel.toUpperCase() + ' ' + s.result.contracts : 'EXPOSURE',
        sub: s.carriedPnL ? Hedge.signedMoney(s.carriedPnL) + ' P/L' :
          (s.cargoContracts ? s.cargoContracts + ' contracts aboard' : 'cash risk aboard'),
